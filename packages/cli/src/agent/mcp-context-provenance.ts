...
function file(options: McpContextProvenanceOptions = {}) { return path.join(path.resolve(options.rootDir ?? process.env.MASTERMIND_PROVIDER_STATE_DIR ?? process.env.WORKBENCH_PROVIDER_STATE_DIR ?? path.join(process.cwd(), '.mastermind-provider-state')), WORKBENCH_MCP_CONTEXT_PROVENANCE_FILENAME) }
